...
/**
 * @event DbgpSession#ready
 * @description
 *
 * Emitted when the session is ready to send and receive commands.
 */

/**
 * @event DbgpSession#end
 * @description
 *
...
/**
 * @event DbgpSession#error
 * @description
 *
 * Emitted when an unexpected error occurs. The `close` event will be emitted
 * directly following this event.
 */

DbgpSession.prototype.write = function(xml) {
    xml = xmlToObject(xml._el);
...
DbgpSession.prototype._handleInit = function(init) {
    ...
    // run or break on line zero

    var _this = this
        , next = (this.breakOnFirstLine ? this.stepInto : this.run);
 ...
 *
 * @see http://xdebug.org/docs-dbgp.php#source
 */
DbgpSession.prototype.getSource = function(fileURI, callback) {
    ...
    _this.sendCommand("source", params, null, function(err, response) {
...
DbgpSession.prototype.getFeature = function(feature, callback) {
    ...
    _this.sendCommand("feature_get", params, null, function(err, response) {
        if (err) return callback(err);
...
DbgpSession.prototype.setFeature = function(feature, value, callback) {
    ...
    _this.sendCommand("feature_set", params, null, function(err, response) {
        if (err) return callback(err);
 ...
 *
 * @see http://xdebug.org/docs-dbgp.php#stack-get
 */
DbgpSession.prototype.getStackFrames = function(stackDepth, callback) {
    ...
    _this.sendCommand("stack_get", params, null, function(err, response) {
 ...
 * If the context name is omitted, the context with an id zero is used (generally the 'locals' context).
 *
 * @see http://xdebug.org/docs-dbgp.php#context-get
 */
DbgpSession.prototype.getContextProperties = function(stackDepth, contextId, callback) {
    ...
    _this.setFeature("max_depth", 0, function() {
        _this.sendCommand("context_get", params, null, function(err, response) {
 ...
 * @param {function({Error?}, {string})} callback
 * @param {Error?} callback.err
 * @param {string} callback.property The property attributes.
 *
 * @see http://xdebug.org/docs-dbgp.php#property-get
 * @see http://xdebug.org/docs-dbgp.php#property-value
 */
DbgpSession.prototype.getProperty = function(propertyName, stackDepth, contextId, callback) {
    ...
    _this.setFeature("max_depth", 0, function() {
        _this.sendCommand("property_get", params, null, function(err, response) {
            if (err) return callback(err);

            var property = response.property;

            _this.sendCommand("property_value", params, null, function(err, response) {
                property["$"] = response["$"];
 ...
 * @param {function({Error?}, {string})} callback
 * @param {Error?} callback.err
 * @param {string} callback.breakpointId An arbitrary string that uniquely identifies this breakpoint in the debugger engine.
 *
 * @see http://xdebug.org/docs-dbgp.php#property-set
 */
DbgpSession.prototype.setPropertyValue = function(propertyName, stackDepth, contextId, value, callback) {
    ...
    _this.sendCommand("property_set", params, value, function(err, response) {
        if (err) return callback(err);
 ...
 * @param {bool=true} options.enabled If the breakpoint is active or not.
 * @param {string=} options.condition Only break if this expression evaluates to `true`.
 * @param {number=} options.ignoreCount Only break after the breakpoint triggered at least _n_ times.
 *
 * @param {function({Error?}, {string})} callback
 * @param {Error?} callback.err
 * @param {string} callback.breakpointId An arbitrary string that uniquely identifies this breakpoint in the debugger engine.
 *
 * @see http://xdebug.org/docs-dbgp.php#breakpoint-set
 */
DbgpSession.prototype.setBreakpoint = function(fileURI, options, callback) {
    ...
    var params = {
        t: "conditional",
        s: options.enabled ? "enabled" : "disabled",
        f: fileURI,
        n: options.line,
    ...
    _this.sendCommand("breakpoint_set", params, condition, function(err, response) {
 ...
 *
 * @param {string} breakpointId The unique identifier given by `setBreakpoint()`.
 * @param {number=} options.line The line number to break on, starting from line 1.
 * @param {bool=true} options.enabled If the breakpoint is active or not.
 * @param {string=} options.condition Only break if this expression evaluates to `true`.
 * @param {number=} options.ignoreCount Only break after the breakpoint triggered at least _n_ times.
 ...
 * @param {Error?} callback.err
 *
 * @see http://xdebug.org/docs-dbgp.php#breakpoint-update
 */
DbgpSession.prototype.updateBreakpoint = function(breakpointId, options, callback) {
    ...
    if (options.enabled != null) {
        params.s = options.enabled ? "enabled" : "disabled";
    }
    ...
    _this.sendCommand("breakpoint_update", params, condition, function(err, response) {
        callback(err);
    });
}
 ...
 * @param {Error?} callback.err
 *
 * @see http://xdebug.org/docs-dbgp.php#breakpoint-remove
 */
DbgpSession.prototype.removeBreakpoint = function(breakpointId, callback) {
    ...
    _this.sendCommand("breakpoint_remove", params, null, function(err, response) {
        callback(err);
    });
}
